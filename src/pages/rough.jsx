...
export default function BookNow() {
  ...
  const handleTabClick = (idx) => {
    ...
    // Measure content width OR fall back to parent width
    const contentWidth = menuEl?.offsetWidth || parentRect.width;
    const maxWidth = parentRect.width; // don't overflow
    const menuWidth = Math.min(contentWidth, maxWidth);

    // Center popover under tab
    const tabCenter = tabRect.left - parentRect.left + tabRect.width / 2;
    // let menuLeft = tabCenter - menuWidth / 2;
    let menuLeft = Math.max(
      0,
      Math.min(tabCenter - menuWidth / 2, parentRect.width - menuWidth),
    );
    ...
    if (menuLeft + menuWidth > parentRect.width) {
      menuLeft = parentRect.width - menuWidth;
    }
        ...
        >
          ...
          {active !== null && (
            <div
              ...
              className={`popover-content absolute top-20 w-full ${active === 0 ? "w-2xl" : ""} ${active === 1 ? "w-xl" : ""} ${active === 2 ? "w-xl" : ""} ${active === 3 ? "w-2xl" : ""} pt-6 transition-all duration-300 ${popoverHeight === 0 ? "scale-50 opacity-0" : "scale-100 opacity-100"}`}
